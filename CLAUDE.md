...
### 2026-04-15: 네비게이션 바와 광고 위치 수정

**문제:**
- 앱에서 광고가 최하단, 네비게이션 바가 광고 위에 배치됨
- 원래 요구사항과 반대 (네비게이션 최하단, 광고가 그 위)

**수정:**
- **네비게이션 바**: 화면 최하단 고정 (marginBottom 제거)
- **광고 (앱)**: AdMob margin을 64px (네비게이션 높이)로 설정하여 네비게이션 바로 위에 배치
- **광고 (웹)**: 기존 유지 (네비게이션 바로 위 고정)
- 앱/웹 레이아웃 일관성 확보

**파일 변경:**
- 수정: `lib/admob.ts` (margin 설정)
- 수정: `components/site-header.tsx` (marginBottom 제거)
- 수정: `components/bottom-ad-container.tsx` (safe-area 계산 개선)
